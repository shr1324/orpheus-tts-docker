...
# v2.0.0: AWQ 4-bit 量化模型
MODEL_CONFIGS = {
    "medium-3b": "Hariprasath28/orpheus-3b-4bit-AWQ",
    "small-1b": "Hariprasath28/orpheus-3b-4bit-AWQ",
    "tiny-400m": "Hariprasath28/orpheus-3b-4bit-AWQ",
    "nano-150m": "Hariprasath28/orpheus-3b-4bit-AWQ"
}
...
def load_model(model_name):
    return OrpheusModel(
        ...
        quantization="awq",  # v2.0.0: 使用 AWQ 4-bit 量化
        dtype="float16"  # AWQ 需要 float16
    )

# 预加载模型到显存
def preload_models():
    """启动时预加载默认模型到显存"""
    print("🚀 Preloading AWQ 4-bit model to GPU...")
    start_time = time.time()
    try:
        model = gpu_manager.get_model("medium-3b", lambda: load_model("medium-3b"))
        elapsed = time.time() - start_time
        print(f"✅ AWQ 4-bit model preloaded in {elapsed:.2f}s")
        return True
...
def generate():
    ...
    try:
        ...
        response.headers['X-Model-Version'] = "v2.0.0-awq-4bit"
