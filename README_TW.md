...
[English](README.md) | [繁體中文](README_CN.md) | [繁體中文](README_TW.md) | [日本語](README_JP.md)
...
- 📊 **API 文件**：内置 Swagger UI
- 🎤 **8种語音选项**：tara, leah, jess, leo, dan, mia, zac, zoe
...
- **精度**：bfloat16（全精度）
- **参数量**：3B（30亿参数）
- **顯存占用**：~39GB（gpu_memory_utilization=0.7）
- **性能指标**：
  - 首次请求：~48秒（模型加载）
  - 后续请求：~2.5秒
  - 流式延迟：~200ms
...
- NVIDIA GPU，顯存 40GB+（如 L40S、A100）
- CUDA 12.1+ 兼容驱动
...
|------|------|------|
| `/health` | GET | 健康檢查 |
| `/api/generate` | POST | 產生語音 |
| `/api/voices` | GET | 列出可用語音 |
...
可用的 MCP 工具：
- `generate_speech`：从文本產生語音
- `get_gpu_status`：檢查 GPU 記憶體使用
- `offload_gpu`：释放 GPU 記憶體
- `list_models`：列出可用模型
...
参见 [DOCKER_DEPLOYMENT.md](DOCKER_DEPLOYMENT.md) 了解 Nginx 反向代理和 SSL 設定。
...
本项目采用 MIT 授權條款 - 详见 [LICENSE](LICENSE) 文件。
